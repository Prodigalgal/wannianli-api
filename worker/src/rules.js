const CANONICAL = "A_IMPERIAL_CANON";
...
const STEM_ACTIVITY = ["开仓库", "栽种", "修灶", "剃头", "受田", "破券", "经络", "合酱", "汲水", "词讼"];
...
function addAuspiciousGodRules(hits, almanac) {
  for (const god of almanac.gods.auspicious) {
    if (["天德", "月德", "天德合", "月德合"].includes(god)) {
      hits.push(ruleHit(`VIRTUE_${god}`, god, "吉神", XIEJI_VOLUME_10, CANONICAL, 3, 0,
        VIRTUE_RECOMMENDS, activities("畋猎,取鱼"), `命中${god}起例`, "卷十列为上吉；忌畋猎取鱼以免伤生气。"));
    } else if (god === "天赦") {
      hits.push(ruleHit("HEAVENLY_PARDON", "天赦", "吉神", XIEJI_VOLUME_10, CANONICAL, 3, 0,
        VIRTUE_RECOMMENDS, activities("畋猎,取鱼"), "命中四时天赦干支", "天地合德、四时旺辰，能解诸凶；不用于出师。"));
    } else if (god === "母仓") {
...
function disposition(recommends, avoids, grade, virtue, allAvoided, retainedTaboo) {
...
function rationale(recommends, avoids, result, grade, virtue, allAvoided, canonical, retainedTaboo) {
...
function resolve(hits, grade, virtuePresent, allActivitiesAvoided) {
  ...
  for (const [activity, value] of evidence) {
    ...
    const result = disposition(recommendBy, avoidBy, grade, virtuePresent, allActivitiesAvoided, retainedTaboo);
    if (result === "RECOMMENDED") recommended.push(activity);
    else if (result === "AVOID") avoided.push(activity);
    else caution.push(activity);
    decisions.push({ activity, disposition: result, conflict: recommendBy.length > 0 && avoidBy.length > 0,
      ...
      rationale: rationale(recommendBy, avoidBy, result, grade, virtuePresent, allActivitiesAvoided, hasCanonical, retainedTaboo) });
  }
  return {
    ...
    policy: {
      ...
      supplementalRule: "低证据传统只补充官修主规则未裁定的活动，不参与反向覆盖。",
    },
  };
}

export function evaluateActivities(date, month, day, almanac) {
